...
static Vector2 GetGridPosition(Vector2 world_pos) {
    return (Vector2){
        floorf(world_pos.x / GRID_SIZE) * GRID_SIZE + GRID_SIZE/2,
...
static void UpdateTowers(void) {
    ...
    for (int i = 0; i < MAX_TOWERS; i++) {
        ...
        if (tower->last_shot < 1.0f / tower->fire_rate) continue;
        ...
        if (closest_enemy >= 0) {
            for (int k = 0; k < MAX_PROJECTILES; k++) {
                if (!game.projectiles[k].active) {
                    ...
                    game.projectiles[k].velocity = (Vector2){direction.x * 400, direction.y * 400};
...
static void UpdateProjectiles(void) {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        ...
        if (proj->position.x < 0 || proj->position.x > 1200 || 
            proj->position.y < 0 || proj->position.y > 800) {
            proj->active = false;
            continue;
        }
...
void DrawGame(void) {
    ...
    for (int i = 0; i < MAX_TOWERS; i++) {
        if (game.towers[i].active) {
            ...
            DrawCircleLines((int)game.towers[i].position.x, (int)game.towers[i].position.y, 
